...
function Postalreceivedlist ({item,deletedata}) {
    ...
    return (
        ...
        <List.Item
            title={()=> (
                <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <Text style={{flex: 1, fontSize: 18}}>{item?.to}</Text>
            ...
            description={()=>(
                ...
                <Text style={{fontSize: 13, color: '#abc',  marginLeft: 30}}>
                 Ref: {item?.ref}</Text>
                <Text style={{fontSize: 13,marginLeft: 30}}>From: {item?.from}</Text>
                </>
            )}
        ...
        {visible && (
            <View style={{backgroundColor: '#fff', borderBottomColor: '#000', borderBottomWidth: 1 }}>
                ...
                <Menu.Item style={{marginLeft: 10}} leadingIcon="square-edit-outline" onPress={()=> router.push(`/admin/Frontdesk/create-edit-postal-received?id=${item?.id}`)} title="Edit" />
...
export default Postalreceivedlist;