...
//   };
  ...
  const onSend = useCallback((messages = []) => {
    let from = cuser?.role == "User" ? cuser?.id : id;
    let receipient = cuser?.role == "User" ? id : cuser?.id;
    const { _id, createdAt, text, user,} = messages[0];
    console.log("receipient",receipient);
    console.log("receipient 2",{ _id, createdAt,  text, user, from, receipient});
    console.log("receipient",name);
